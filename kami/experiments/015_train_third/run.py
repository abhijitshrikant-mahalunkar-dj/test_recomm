...
def save_model(cfg: DictConfig, bst: lgb.Booster, output_path: Path, name: int) -> None:
    ...
    if ONNX_AVAILABLE:
        try:
            logger.info(f"Exporting model to ONNX format: {name}")
            num_features = len(bst.feature_name())
            initial_types = [('input', onnxmltools.convert.common.data_types.FloatTensorType([None, num_features]))]
            onnx_model = convert_lightgbm(bst, initial_types=initial_types, target_opset=9)
            onnx_path = output_path / f"model_{name}.onnx"
            onnxmltools.utils.save_model(onnx_model, str(onnx_path))
            logger.info(f"ONNX model saved to: {onnx_path}")
        except Exception as e:
            logger.warning(f"Failed to export ONNX model: {e}")
    else:
        logger.info("ONNX export skipped (onnxmltools not installed)")
